...
Customization:
    ...
    @custom('{custom_regularizer_class_name}')
...
from .samples.regularizer_samples import sample_functions, default_ranges

def randomize(serial, attributes=[], ranges=default_ranges):
    '''
        Description:
            This function is used to randomize regularizer
            attributes in an advanced serial format using
            pre-defined parameters.
        
        Arguments:
            *serial         : A regularizer serial.
...
def custom(class_name, override=False):
    ...
    if class_name in sample_functions and not override:
        raise AttributeError("Cannot override '{}'. Must be a unique regularizer.".format(class_name))
